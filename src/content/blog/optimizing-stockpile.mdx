...
This is one of end of the spectrum (the one we don't want to be on), with the other being creating accounts for every vote. The problems with the v2 implementation are twofold: write-locks and account size. 
...
Perhaps the hardest problem with quadratic funding is the concept or sybil prevention,
...
(so they can rotate if needed), and with Civic . For custom strategies, we use a similar approach to Nifty Asset, and represent custom strategies as serialized instructions.
